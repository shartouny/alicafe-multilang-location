...
use TCG\Voyager\Http\Controllers\VoyagerBaseController as ControllersVoyagerBaseController;

class FinderQuestionsController extends ControllersVoyagerBaseController
{
    ...
    public function edit(Request $request, $id)
    {
        ...
        $answers = FinderAnswers::where('question_id', $id)->get();
    ...
    public function update(Request $request, $id)
    {
        ...
        $question =FinderQuestions::find($id);
        $question->question = $request->question;
        $question->save();
        if($request->answer){
            $i = 0;
            foreach ($request->answer as $v) {
                $id = $request->incoming_answers[$i] ?? '';
                if(!empty($id)){
                    $answer = FinderAnswers::find($id);
                }else{
                    $answer = new FinderAnswers;
                }
                $answer->answer = $v;
                $answer->question_id = $question->id;
                if ($request->hasFile('answerImg')) {
                    if(isset($request->file('answerImg')[$i])){
                        $imgName = $request->file('answerImg')[$i];
                        // dd($request->file('answerImg')[$i]);
                        $fileNameWithExt = $imgName->getClientOriginalName();
                        //get the file name without extention
                        $fileName = pathinfo($fileNameWithExt, PATHINFO_FILENAME);
                        // get the extention without file name 
                        $ext = $imgName->getClientOriginalExtension();
                        // create the filename to store 
                        $fileNameToStore = 'answer-imgs/' . $fileName . '_' . time() . '.' . $ext;
                        $path = $imgName->storeAs('public/', $fileNameToStore);
                        // dd($ext);
                        $answer->img = $fileNameToStore;
                    }
                }
                $answer->save();
                $i++;
        ...
        if($request->deleted_answers){
            foreach($request->deleted_answers as $delete){
                $deleteChoice = FinderAnswers::find($delete);
                if($deleteChoice){

                    if (!empty($deleteChoice->img)) {
                        Storage::delete('answer-imgs/' . $deleteChoice->img);
                    }
                    $deleteChoice->delete();
                }
            }
